 ...
 * camera properties.
 */
typedef struct RemotePixelFormat
{
...
static const uint32_t _preferred_formats[] =
    {
        /* Native format for the virtual camera: no conversion at all. */
        V4L2_PIX_FMT_YUV420,
        V4L2_PIX_FMT_YVU420,
...
/* Frees resource allocated for RemotePixelFormat instance, excluding the instance
 * itself.
 */
static void _remote_pixel_format_free(RemotePixelFormat *fmt)
{
    if (fmt != NULL)
    {
        if (fmt->dims != NULL)
 ...
 */
static int
_get_format_index(uint32_t fmt, RemotePixelFormat *formats, int size)
{
    int f;
 ...
 */
static int
_camera_device_enum_pixel_formats(LinuxCameraDevice *cd, RemotePixelFormat **fmts)
{
    int n, max_fmt;
    int fmt_num = 0;
    int out_num = 0;
    struct v4l2_fmtdesc fmt_enum;
    RemotePixelFormat *arr;
    ...
    *fmts = (RemotePixelFormat *)malloc(fmt_num * sizeof(RemotePixelFormat));
    if (*fmts == NULL)
    {
        E("%s: Memory allocation failure", __FUNCTION__);
        return -1;
    ...
    arr = *fmts;
    memset(arr, 0, fmt_num * sizeof(RemotePixelFormat));
    for (n = 0; n < max_fmt && out_num < fmt_num; n++)
    {
        ...
        if (_xioctl(cd->handle, VIDIOC_ENUM_FMT, &fmt_enum))
        {
            ...
            {
                _remote_pixel_format_free(arr + nn);
            }
            free(arr);
            return -1;
        }
...
_camera_device_get_info(LinuxCameraDevice *cd, CameraInfo *cis)
{
    int f;
    int chosen = -1;
    RemotePixelFormat *formats = NULL;
    ...
    {
        _remote_pixel_format_free(formats + f);
    }
    free(formats);
                             ...
                             float exp_comp)
{
    ...
    else
    {
        ...
        CLEAR(buf);
        buf.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
        buf.memory = cd->io_type == CAMERA_IO_MEMMAP ? V4L2_MEMORY_MMAP : V4L2_MEMORY_USERPTR;
        for (;;)
        {
