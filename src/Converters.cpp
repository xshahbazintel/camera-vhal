...
static void _YUV420SToRGB565(const uint8_t *Y, const uint8_t *U, const uint8_t *V, int dUV,
                             uint16_t *rgb, int width, int height, int y_stride, int uv_stride) {
...
static void _YUV420SToRGB32(const uint8_t *Y, const uint8_t *U, const uint8_t *V, int dUV,
                            uint32_t *rgb, int width, int height, int y_stride, int uv_stride) {
    const uint8_t *Y_pos = Y;
    const uint8_t *U_pos = U;
    const uint8_t *V_pos = V;
...
void YV12ToRGB565(const void *yv12, void *rgb, int width, int height) {
    ...
    _YUV420SToRGB565(Y, U, V, 1, reinterpret_cast<uint16_t *>(rgb), width, height, y_stride,
                     uv_stride);
}
...
void YV12ToRGB32(const void *yv12, void *rgb, int width, int height) {
    ...
    _YUV420SToRGB32(Y, U, V, 1, reinterpret_cast<uint32_t *>(rgb), width, height, y_stride,
                    uv_stride);
}
 ...
 */
static void _NVXXToRGB565(const uint8_t *Y, const uint8_t *U, const uint8_t *V, uint16_t *rgb,
 ...
 * y, u, and v point to Y,U, and V panes, where U and V values are interleaved.
 */
static void _NVXXToRGB32(const uint8_t *Y, const uint8_t *U, const uint8_t *V, uint32_t *rgb,
                         int width, int height) {
    // The UV stride for NV21 and NV12 is the same as the width because the
    // U and V values are interleaved, making each row twice as wide even though
    // each value covers a two pixel wide area. These formats do not require any
...
void NV12ToRGB565(const void *nv12, void *rgb, int width, int height) {
    ...
    _NVXXToRGB565(y, y + pix_total, y + pix_total + 1, reinterpret_cast<uint16_t *>(rgb), width,
                  height);
}
...
void NV12ToRGB32(const void *nv12, void *rgb, int width, int height) {
    const int pix_total = width * height;
    const uint8_t *y = reinterpret_cast<const uint8_t *>(nv12);
    _NVXXToRGB32(y, y + pix_total, y + pix_total + 1, reinterpret_cast<uint32_t *>(rgb), width,
                 height);
}
...
void NV21ToRGB565(const void *nv21, void *rgb, int width, int height) {
    ...
    _NVXXToRGB565(y, y + pix_total + 1, y + pix_total, reinterpret_cast<uint16_t *>(rgb), width,
                  height);
}
...
void NV21ToRGB32(const void *nv21, void *rgb, int width, int height) {
    const int pix_total = width * height;
    const uint8_t *y = reinterpret_cast<const uint8_t *>(nv21);
    _NVXXToRGB32(y, y + pix_total + 1, y + pix_total, reinterpret_cast<uint32_t *>(rgb), width,
                 height);
}
